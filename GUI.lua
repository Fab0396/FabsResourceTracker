...
local function BuildPageWindows(page, onWindowsChanged)
    ...
    newBtn:SetScript("OnClick",function()
        local d = ConsumableTrackerDB; if not d or not d.Windows then return end
        local newWin = {}
        -- Copy current selected window's appearance settings (not slots, not position)
        local srcWin = d.Windows[_selectedWinIdx] or d.Windows[1]
        for k,v in pairs(srcWin) do
            if k~="Slots" and k~="X" and k~="Y" and k~="Name" then newWin[k]=v end
        end
        newWin.Name = "Window "..(#d.Windows+1)
        newWin.Slots = {}
        newWin.X = 0
        newWin.Y = 0
        newWin.AnchorPoint = "CENTER"
        newWin.AnchorToFrame = "UIParent"
        newWin.AnchorToPoint = "CENTER"
        newWin.Locked = false
...
local function BuildGUI()
    ...
    local pageH={1350,1600,1400,1800,300,600,2000,2800}
    local activePage=1
